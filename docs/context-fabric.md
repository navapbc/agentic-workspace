...
|---|---|---|---|
| **System resource** | `system:<slug>` | A shared system and how to reach it (a GitHub org, a Jira, a data warehouse), stable interface metadata | Credentials, product ownership, maintainers |
...
Templates for all three are in `../templates/context-fabric/`.
...
- lists the **systems** it uses by `systemId`, with validated **anchors** (for example, the exact Jira project key or Confluence page ID that is the product's home),
...
1. Copy the relevant templates from `../templates/context-fabric/`.
