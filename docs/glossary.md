...
| **Product-work layer** | The actual product artifacts, split into canonical and prototyping lanes. |
| **`AGENTS.md`** | The canonical, cross-tool agent-instructions file. Thin. Boundaries and routing only. |
...
| **Adapter** | A thin per-harness pointer at `skills/<name>/adapters/<harness>/` that references the canonical `SKILL.md` and never forks it. |
| **Sync (skills)** | Mirroring canonical skills into each harness's runtime directory via a script; `--check` detects drift. |
...
| **Canonical** | Reviewed, source-of-truth work: in the canonical lane, human-reviewed, with a named steward. |
| **Prototype / prototyping lane** | Exploratory work: spikes, mockups, experiments. Pre-review, no assigned profile. |
| **Steward** | The named owner of a product area and its human review gate. |
...
| **Docs lanes** | Standard subfolders under a workspace `docs/`: `ideation/`, `plans/`, `solutions/`, and reports. |
