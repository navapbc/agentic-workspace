...
The three phases map onto Nava's AI-strategy maturity horizons (pilot → repeatable patterns → scaling), so a team's workspace maturity is legible to the broader transformation effort.

| Phase | You have | Effort to enter | Nava strategy horizon |
|---|---|---|---|
| **Crawl** | One workspace, thin `AGENTS.md`, standard Docs lanes | ~30 min | Phase 1 — pilot-level shift |
...
4. Use the Docs lanes as you work: `docs/ideation/`, `docs/plans/`, `docs/solutions/`. Don't invent new lanes yet.
...
6. **Split the lanes.** Separate canonical product work from prototyping.
...
1. **Complete the support layer:** `tools/` for approved mechanics, `validation/` for invariants, `docs/architecture.md` for your specific instantiation.
2. **Adopt validation in your loop.** Run `scripts/validate-workspace.sh` after structural changes and, if git-backed, in CI.
3. **Formalize stewardship.** Name a steward per product area (in the profile's `maintainers` and a stewardship summary). Define the review gate that makes something canonical.
...
5. **Add expert / user review panels** where domain correctness matters (personas an agent can adopt, plus human advisors).
...
- **Skills that fork per tool.** Editing the tool-specific adapter instead of the canonical `SKILL.md`. The adapter is a pointer; the procedure has one home.
