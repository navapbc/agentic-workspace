...
- [x] `schemas/shared/1/defs.json` — identifier, upstream reference, location
...
- [x] `schemas/org/1/schema.json`
- [x] `schemas/bounded-context/1/schema.json`
- [x] `schemas/individual/1/schema.json`
...
- [x] One valid fixture per tier.
- [x] One invalid fixture per schema-detectable rule, named by its finding code
      lowercased with `_` replaced by `-`.
- [x] An evasion corpus for denylist near-misses.
...
- [x] `scripts/render-templates.sh` with `--check`.
- [x] `templates/org.TEMPLATE.yaml`, `templates/bounded-context.TEMPLATE.yaml`,
...
- [x] `tests/migrations.test.sh` — for every tier and every contract version
...
- [x] Frozen-hash assertion over every released contract directory.
...
- [x] Pin `check-jsonschema` in `framework.json` and give the schema stage a
      named skipped-stage code.
- [x] Add `openspec/` to the denylist and real-name screening scopes — the
      tree is public and carries reasoning drawn from documents that are not.
- [x] `tests/openspec.test.sh` — version pin matches, and every
