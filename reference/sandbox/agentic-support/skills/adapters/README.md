...
`skills/<skill-name>/` is the source-of-truth skill bundle. Keep the shared `SKILL.md`, `references/`, and `assets/` there.
...
- An adapter is a **thin pointer**, not a copy. It names the harness and, if needed, that harness's discovery convention, then defers to the source-of-truth `SKILL.md`.
- An adapter must state: "The source-of-truth workflow controls behavior whenever this adapter is stale or disagrees with it."
- Do not fork shared workflow instructions into an adapter. If you find yourself copying the procedure, stop — put it in the `SKILL.md`.
...
This {{Harness}} skill is a thin adapter. Read the source of truth at
